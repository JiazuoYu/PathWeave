...
class Blip2VicunaXInstruct(Blip2Base):
    ...
    def __init__(
        ...
        modalities = ["image", "pc", "audio", "video", "depth"],
        use_cues=True,
        num_query_token=32, 
        ...
        projection_only_depth=False,

        projection_path_audio=False,
        projection_path_pc=False,
        projection_path_video=False,
        projection_path_image=False,
        projection_path_depth=False,
        
        Evaluate_stage=False,
        special_module_path=None,
        special_module_type="",
        freeze_qformer_LLM=False,
        adapter_for_finetune=False,
        L2_loss=False,
        ...
        logging.info(
            f"==================================================================================")
        logging.info(
            f"=========================== Adapter in  adpater for xx modality ===================================")
        logging.info(
            f"==================================================================================")
        self.is_training = not Evaluate_stage
        logging.info(
            f"=============================train flag  {self.is_training}======================================")
        ...
        logging.info(f"use L2loss: {self.l2_loss}")

        self.adapter_for_finetune = adapter_for_finetune
        logging.info(f"use adapter：{self.adapter_for_finetune}")
        
        self.use_mlp = use_mlp
        logging.info(f"use mlp in adapter: {self.use_mlp}")
        if self.use_mlp:
            self.mlp_type=mlp_type
            logging.info(f"mlp type: {self.mlp_type}")
        else:
            self.mlp_type=None
        ...
        for modality in self.modalities:
            ...
            # special_module的path和type
            if Evaluate_stage is False:  # training stage
                modality_kwargs['load_ln_path'] = locals()[f"pretrained_{modality}_qformer"]
                modality_kwargs['load_ln_type'] = locals()[f"load_ln_type_{modality}"]
                special_query_path=None # w/o query
                special_EncProj_path=None
                # ln_and_EncProj_path =
                special_type = special_module_type
            #
            else:
                modality_kwargs['load_ln_path'] = special_module_path
                modality_kwargs['load_ln_type'] = special_module_type
                special_query_path=special_module_path
                special_EncProj_path = special_module_path
                special_type = special_module_type
            ...
            if self.projection_only or locals()[f"projection_only_{modality}"]: # false
            ...
            ######### Init Different Modality Encoder ############
        ...
        for modality in self.modalities:
                ...
                if modality in 'pc' or modality in 'audio':
                    modality_num_features = 1408
                    logging.info(f"pc or audio num feature维度为{modality_num_features}")
                ...
                modality_qformer, modality_query_tokens = self.init_Qformer(
                    ...
                    load_special_query_path=special_query_path,
                    special_query_type=special_type,
                    current_modality=modality,
                    adapter_for_finetune=adapter_for_finetune,
                    is_training=self.is_training
                )
                self.freeze_qformer_LLM=freeze_qformer_LLM
                if self.freeze_qformer_LLM:
                    logging.info(f"frozen qformer")
                    for k, v in modality_qformer.named_parameters():
                        # print(k)
                        if "adapter" in k:
                            v.requires_grad = True
                        else:
                            v.requires_grad = False # frozen
                        
                        if modality in 'audio' and  "adapter_list.0" in k: # frozen previous adapters
                            v.requires_grad = False

                        if modality in 'depth' and ("adapter_list.0" in k or "adapter_list.1" in k):
                            v.requires_grad = False
                        
                        if modality in 'pc' and ("adapter_list.0" in k or "adapter_list.1" in k or "adapter_list.2" in k):
                            v.requires_grad = False
                ...
                if modality in 'audio' or modality in 'pc':
                    modality_encoder_features = getattr(self, f"{modality}_encoder").num_features  # 512
                    logging.info(f"current modality: {modality}，load EncProj dim")
                    setattr(self, f"{modality}_encoder_projection", self.init_encoder_projection(modality_encoder_features, modality_num_features,
                                                         special_EncProj_path, special_type))
        ...
        for modality in self.modalities:
            ...
            if self.projection_only or getattr(self, f"projection_only_{modality}"):
                ...
                if self.adapter_for_finetune:
                    proj2_list = self.init_vicuna_projection_adapter(
                        qformer.config.hidden_size,
                        self.llm_hidden_size,
                        load_projection_path=load_projection_path,
                        load_projection_type=load_projection_type,
                        freeze_qformer_LLM=freeze_qformer_LLM,
                        order_dic=self.order_dic
                    )
            if self.freeze_qformer_LLM:
                logging.info(f"frozen llm_projection")
                for k, v in proj.named_parameters():
                    # print(k)
                    v.requires_grad = False

            setattr(self, f"{modality}_llm_proj", proj)
            if self.adapter_for_finetune:
                setattr(self, f"{modality}_llmProj_adapter_list", proj2_list)

            if (modality in 'audio' or modality in 'pc' ) and self.adapter_for_finetune:
                for k, v in getattr(self, f"{modality}_llmProj_adapter_list").named_parameters():
                    if '0.' in k and modality in 'audio':
                        v.requires_grad = False
                    elif modality in 'pc' and ('0.' in k or '1.' in k or '2.' in k):
                        v.requires_grad = False
            ...
            # l2
            if self.l2_loss:
                logging.info(f'qformer llm_proj l2 loss')
                modality_qformer_ref_l2 = copy.deepcopy(modality_qformer)
                proj_ref_l2 = copy.deepcopy(proj)
                for k, v in modality_qformer_ref_l2.named_parameters():
                    # print(k)
                    v.requires_grad = False
                for k, v in proj_ref_l2.named_parameters():
                    # print(k)
                    v.requires_grad = False
                setattr(self, f"{modality}_Qformer_ref_l2", modality_qformer_ref_l2)
                setattr(self, f"{modality}_llm_proj_ref_l2", proj_ref_l2)
    ...
    def forward(self, samples):
        ...
        for modality in curr_modalities:
            ...
            ln = getattr(self, f"{modality}_ln") # video 1408
            ...
            if modality == "video" and self.video_enc_name in Blip2VicunaXInstruct.SEQUENCIAL_ENCODERS:
                ...
                for j in range(data.size(1)):
                    ...
                    with self.maybe_autocast():
                        embeds[modality].append(ln(encoder(this_frame)))
                        # print('11，', embeds[modality][j].dtype)
                        embeds[modality][j] = getattr(self, f"{modality}_encoder_projection")(embeds[modality][j])
                        # print(getattr(self, f"{modality}_encoder_projection").weight.dtype)
                        # print('22，', embeds[modality][j].dtype)
                    data_atts[modality].append(torch.ones(embeds[modality][j].size()[:-1], dtype=torch.long).to(self.device))
                # B, Token Size, LM EMB
                if not self.projection_only and not getattr(self, f"projection_only_{modality}"):
                    query_tokens[modality] = getattr(self, f"{modality}_query_tokens").expand(data.size(0), -1, -1)
            elif modality == "depth" and self.depth_enc_name in Blip2VicunaXInstruct.SEQUENCIAL_ENCODERS:
                embeds[modality] = []
                data_atts[modality] = []
                for j in range(data.size(2)):
                    this_frame = data[:,:,j,:,:]
                ...
                # B, C
                if modality in 'pc':
                    # print(embeds[modality].dtype)
                    embeds[modality] = getattr(self, f"{modality}_encoder_projection")(
                        embeds[modality].to(torch.float32))
                    # print("!!",embeds[modality].shape)
            ...
            for modality in curr_modalities:
                ...
                if modality in Blip2VicunaXInstruct.SEQUENCIAL_MODALITIES and getattr(self, f'{modality}_enc_name') in Blip2VicunaXInstruct.SEQUENCIAL_ENCODERS:
                    ...
                    query_output = getattr(self, f"{modality}_Qformer").bert(
                        ...
                        encoder_hidden_states=reordered_embeds.to(torch.float32),
                    ...
                    query_outputs[modality] = getattr(self, f"{modality}_Qformer").bert(
                        ...
                        encoder_hidden_states=embeds[modality],
                    ...
                    if modality in "audio":
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                               + getattr(self, f"{modality}_llmProj_adapter_list")[
                                                   self.order_dic[modality] - 1](
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                               + getattr(self, f"{modality}_llmProj_adapter_list")[
                                                   self.order_dic[modality]](
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :])
                    if modality in "pc":
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[self.order_dic[modality]-3](query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[self.order_dic[modality]-2](query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[self.order_dic[modality]-1](query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[self.order_dic[modality]](query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :])
                ...
                if self.adapter_for_finetune:
                    ...
                    if modality in "pc":
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[
                                                   self.order_dic[modality] - 3](
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[
                                                   self.order_dic[modality] - 2](
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
                                                + getattr(self, f"{modality}_llmProj_adapter_list")[
                                                   self.order_dic[modality] - 1](
                            query_outputs[modality].last_hidden_state[:, :query_tokens[modality].size(1), :]) \
        ...
        if self.l2_loss:
            for modality in curr_modalities:
                model_qformer = getattr(self, f'{modality}_Qformer')
                model_proj = getattr(self, f"{modality}_llm_proj")
                model_qformer_ref_l2 = getattr(self, f'{modality}_Qformer_ref_l2')
                model_proj_ref_l2 = getattr(self, f"{modality}_llm_proj_ref_l2")
            l2_loss_qformer = self.l2_loss_reg(model_qformer, model_qformer_ref_l2)
            l2_loss_proj = self.l2_loss_reg(model_proj, model_proj_ref_l2)
            loss = loss + l2_loss_qformer + l2_loss_proj
    ...
    def init_depth_encoder(
        ...
        if model_name in  ["eva_clip_g","eva2_clip_L","clip_L",]:
            depth_encoder, ln_depth =  self.init_image_encoder(
                model_name, precision=precision, **kwargs
            )
        self.depth_enc_name = model_name
        return depth_encoder, ln_depth
    ...
    def generate(
        ...
        if 'modalities' in samples:
            ...
            curr_modalities = ["video", "audio", "depth"]
        else:
            curr_modalities = [modality for modality in self.modalities if modality in samples]
                ...
                for j in range(data.size(2)):
                    ...
                    with self.maybe_autocast():
                        ...
                        if self.shared_qformer:
                            embeds[modality][j] = getattr(self, f"{modality}_encoder_projection")(embeds[modality][j])
                        data_atts[modality].append(torch.ones(embeds[modality][j].size()[:-1], dtype=torch.long).to(self.device))
            elif modality == "depth" and "clip" in self.depth_enc_name:
                embeds[modality] = []
                data_atts[modality] = []
                for j in range(data.size(2)):
                    this_frame = data[:,:,j,:,:]
        ...
        if self.joint_video_audio:
            for pos in range(num['video']):
                ...
                if self.use_cues:
                    for modality in ['video', 'audio', 'depth']:
                        if self.clean_tokenization:
                            if self.prefix or pos > 1 or self.enumerate_inputs or modality == 'audio' or modality == 'depth':
    ...
    def from_config(cls, cfg):
        ...
        depth_model = cfg.get("depth_model","eva_clip_g")
        ...
        pretrained_depth_qformer = cfg.get("pretrained_depth_qformer",None)
        ...
        load_attention_depth_qformer = cfg.get("load_attention_depth_qformer",False)
        ...
        load_qformer_type_depth=cfg.get('load_qformer_type_depth', "")
        ...
        load_projection_depth=cfg.get('load_projection_depth', True)
        ...
        load_projection_type_depth=cfg.get('load_projection_type_depth', "")
        ...
        load_ln_type_depth=cfg.get('load_ln_type_depth', "")
        ...
        depth_encoder_kwargs = cfg.get("depth_encoder_kwargs",{})
        ...
        depth_precision = cfg.get("depth_precision","fp16")
        ...
        freeze_depth = cfg.get("freeze_depth",True)
        ...
        encoder_projection_type_depth=cfg.get("encoder_projection_type_depth","")
        ...
        cached_depth= cfg.get("cached_depth", False)
        ...
        num_features_depth=cfg.get('num_features_depth', 14080)
        ...
        projection_only_depth=  cfg.get('projection_only_depth', False)
        ...
        projection_path_depth=cfg.get('projection_path_depth', False)
        
        remove_start=cfg.get('remove_start', False)
        proj_dim=cfg.get('proj_dim', 1)
        clean_tokenization=cfg.get('clean_tokenization', False)
        Evaluate_stage=cfg.get('Evaluate_stage', False)
        special_module_path=cfg.get('special_module_path', None)
        special_module_type=cfg.get('special_module_type', "")
        freeze_qformer_LLM=cfg.get('freeze_qformer_LLM', False)
        adapter_for_finetune=cfg.get('adapter_for_finetune', False)
        L2_loss=cfg.get('L2_loss', False)
        use_mlp=cfg.get('use_mlp', False)
        mlp_type = cfg.get('mlp_type', '')
        ...
        model = cls(
            ...
            pretrained_depth_qformer=pretrained_depth_qformer,
            ...
            load_attention_depth_qformer=load_attention_depth_qformer,
            ...
            load_qformer_type_depth=load_qformer_type_depth,
            ...
            load_projection_depth=load_projection_depth,
            ...
            load_projection_type_depth=load_projection_type_depth,
            ...
            load_ln_type_depth=load_ln_type_depth,
            ...
            encoder_projection_type_depth=encoder_projection_type_depth,
            ...
            projection_path_depth=projection_path_depth,
            ...
            num_features_depth=num_features_depth,
            ...
            projection_only_depth=projection_only_depth,
            ...
            special_module_path=special_module_path,
            special_module_type=special_module_type,
            freeze_qformer_LLM=freeze_qformer_LLM,
            adapter_for_finetune=adapter_for_finetune,
            L2_loss=L2_loss,
            use_mlp=use_mlp,
        ...
        if load_ln_path and load_ln_type:
            ...
            for k in checkpoint.keys():
                if load_ln_type in k:
                    if str(num_features) not in str(checkpoint[k].shape):
                        logging.info(
                            f"unmatch size， donot load ln,ln {num_features}，ckpt{checkpoint[k].shape}")
                        continue
                    logging.info(
                        f"Loading pretrained layer norm weights from {url_or_filename} of type {load_ln_type},params name：{k}")
    ...
    def init_encoder_projection(cls, enc_num_features, shared_qformer_num_features, load_proj_path=False, load_proj_type=""):
        ...
        if load_proj_path and load_proj_type:
            ...
            logging.info(
                f"test， {url_or_filename}， type： {load_proj_type}， dim：{enc_num_features}——>{shared_qformer_num_features}")
            if is_url(url_or_filename):
                cached_file = download_cached_file(
                    url_or_filename, check_hash=False, progress=True
                )
            ...
            logging.info(
                f"training，random init encoder projection ， dim：{enc_num_features}——>{shared_qformer_num_features}")
            print(encoder_projection.weight.dtype)
    ...
    @classmethod
    def init_vicuna_projection_adapter(cls, input_size, output_size, load_projection_path=False, load_projection_type="",
                               projection_key=None, freeze_qformer_LLM=False, order_dic={}):
        # order_dic = order_dic
        if freeze_qformer_LLM:
            adapter_layer_list_llm_proj = nn.ModuleList()
            for i in range(4):
                adapter_llm_proj = Adapter(d_model=input_size, dropout=0.1, bottleneck=8,
                                           init_option='lora',
                                           adapter_scalar='learnable_scalar',
                                           adapter_layernorm_option='none',
                                           d_model_2=output_size)
                adapter_layer_list_llm_proj.append(adapter_llm_proj)
            # proj2 = adapter_layer_list_llm_proj[order_dic[modality]]
        if load_projection_path:
            url_or_filename = load_projection_path
            logging.info(
                f"Loading pretrained LLM_adapter weights from {url_or_filename} of type {load_projection_type} with key {projection_key if projection_key else load_projection_type + '_llmProj_adapter.'}")
            if is_url(url_or_filename):
                cached_file = download_cached_file(
                    url_or_filename, check_hash=False, progress=True
                )
            ...
            for k in checkpoint.keys():
                if projection_key:
                    if projection_key in k:
                        loaded_state_dict['.'.join(k.split('.')[1:])] = checkpoint[k]
                        logging.info(
                            f"{loaded_state_dict['.'.join(k.split('.')[1:])]}")
                else:
                    if load_projection_type + 'llmProj_adapter_list.' in k:
                        loaded_state_dict['.'.join(k.split('.')[1:])] = checkpoint[k]
            adapter_layer_list_llm_proj.load_state_dict(loaded_state_dict, strict=False)

        return adapter_layer_list_llm_proj

    @classmethod
    def init_Qformer(cls, num_query_token, modality_width, cross_attention_freq=2, use_mlp=False, mlp_type='base', pretrained_qformer=None, load_attention=False, load_qformer_type="", load_special_query_path=None, special_query_type="", current_modality="", adapter_for_finetune=False, is_training=True):
        logging.info(f"modality_width，{modality_width}")
        encoder_config = BertConfig.from_pretrained("/15324359926/Multimodal/bert-base-uncased")
        ...
        encoder_config.is_training = is_training
        if adapter_for_finetune == True:
            logging.info(f"adapter")
            encoder_config.lora_bottleneck = 8
            encoder_config.current_modality = current_modality
            encoder_config.adapter_for_finetune = adapter_for_finetune
            if use_mlp:
                logging.info("MLP in adapter")
                encoder_config.use_mlp = use_mlp
                encoder_config.mlp_type = mlp_type
            elif not use_mlp:
                encoder_config.use_mlp = False
                encoder_config.mlp_type = None
        # 是用encoder_config传参
        ...
        if pretrained_qformer:
            ...
            logging.info(f"Loading pretrained qformer weights from {url_or_filename} of type {load_qformer_type}")
            if is_url(url_or_filename):
                cached_file = download_cached_file(
                    url_or_filename, check_hash=False, progress=True
                )
            ...
            if load_qformer_type:
                load_qformer_type = f"{load_qformer_type}_" # 不用改这个地方的代码
            loaded_state_dict = {}
            if 'model' in checkpoint:
                checkpoint = checkpoint['model'] 
            for k in checkpoint.keys():
                if load_qformer_type+'Qformer.' in k: # 对应的模态的
                    if not load_attention and 'attention' in k: # 不会走这里
                        continue
                    loaded_state_dict['.'.join(k.split('.')[1:])] = checkpoint[k]
            Qformer.load_state_dict(loaded_state_dict, strict=False)

        # yjz:为special token设计
        if load_special_query_path:
            url_or_filename_special = load_special_query_path
            logging.info(
                f"test：{url_or_filename_special} load speical，type {special_query_type}")
            if is_url(url_or_filename_special):
                cached_file = download_cached_file(
                    url_or_filename_special, check_hash=False, progress=True
                )
                checkpoint_special = torch.load(cached_file, map_location="cpu")
            elif os.path.isfile(url_or_filename_special):
                checkpoint_special = torch.load(url_or_filename_special, map_location="cpu")
            else:
                raise RuntimeError("checkpoint url or path is invalid")
            if special_query_type:
                special_query_type = f"{special_query_type}_"
            if 'model' in checkpoint_special:
                checkpoint_special = checkpoint_special['model']
            query_tokens.data = checkpoint_special[special_query_type+'query_tokens']

        else:
            logging.info(
                f"training：load {url_or_filename} type {load_qformer_type}")
    ...
    def l2_loss_reg(self, model, model_ref):
        loss = 0.0
