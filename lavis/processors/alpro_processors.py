...
@registry.register_processor("alpro_depth_train")
class AlproDepthTrainProcessor(BlipImageBaseProcessor):
    def __init__(
        self,
        image_size=384,
        mean=None,
    ...
    def __call__(self, rgb_path, depth_path, start_sec=None, end_sec=None):
        """
        Args:
            clip (torch.tensor): Video clip to be cropped. Size is (C, T, H, W)
        Returns:
        ...
        x_image = Image.open(depth_path).convert('RGB')
        
        image_transformed = self.transform(image).unsqueeze(1)
        x_transformed  = self.transform(x_image).unsqueeze(1)
        

        transformed = torch.cat([image_transformed, x_transformed], dim=1)
        # print('transformers:', transformed.shape)
...
@registry.register_processor("alpro_depth_val")
class AlproDepthEvalProcessor(BlipImageBaseProcessor):
    def __init__(
        self,
        image_size=384,
        mean=None,
    ...
    def __call__(self, rgb_path, depth_path, start_sec=None, end_sec=None):
        ...
        # print('image_transformed:', image_transformed.shape)
