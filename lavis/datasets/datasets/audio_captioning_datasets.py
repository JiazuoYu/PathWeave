...
class AudioSetDataset(AudioCaptioningDataset):
    def __init__(self, **kwargs):
        ...
        self.sample_ids = set.intersection(*[set(getattr(self, f"existing_{modality}_annotation")) for modality in self.modalities])  # todo 为什么只加载61条数据
    ...
    def get_audio_path(self, ann):
        ...
        return str(os.path.realpath(os.path.join(self.audio_root, ann[self.sample_id_key] + '_{:.1f}.wav'.format(ann['start_seconds'], ann['end_seconds'])))).replace('all_audio/', '')
...
class AudioCapsDataset(AudioCaptioningDataset):
    def __init__(self, **kwargs):
        ...
        def get_existing_audio_annotations(self):
            return [f.split('.')[0] for f in os.listdir(self.audio_root)] if not self.cached else [f.split('_')[0] for f in os.listdir(self.cached_dir)]

        self.sample_ids = set.intersection(*[set([f.split('.')[0] for f in os.listdir(self.audio_root)] if not self.cached else [f.split('_')[0] for f in os.listdir(self.cached_dir)]) for modality in self.modalities])  # 获取不重复的路径名？
    ...
    def get_audio_path(self, ann):
        ...
        return os.path.join(self.audio_root, ann[self.sample_id_key] + '.wav')
...
class AudioLanguagePretrainDataset(BaseDataset, __DisplMixin):
    def __init__(self, **kwargs):
        ...
        self.cached = kwargs.get('cached', False) # True
        self.cache_dir = kwargs.get('cached_dir', '')
