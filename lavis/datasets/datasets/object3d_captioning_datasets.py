...
class Object3dCaptionDataset(BaseDataset, __DisplMixin):
    def __init__(self, **kwargs):
        ...
        self.annotation = [ann for ann in self.annotation if ann['sample_id'] + '_8192.npz' in self.sample_ids]
...
class ObjaverseCaptionDataset(Object3dCaptionDataset, __DisplMixin):
    ...
    def get_existing_pc_annotations(self):
        # a  = list(set(os.listdir(self.pc_root)).intersection(set(ann['sample_id'] + '_8192.npz' for ann in self.annotation)))
        # print(a)
        return list(set(os.listdir(self.pc_root)).intersection(set(ann['sample_id']+ '_8192.npz' for ann in self.annotation)))

    def get_pc_path(self, sample_key):
        return os.path.join(self.pc_root, '{}_{}.npz'.format(sample_key, self.npoints))
