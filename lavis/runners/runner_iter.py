...
class RunnerIter(RunnerBase):
    ...
    def train(self):
        ...
        for start_iters in range(
            ...
            # self._save_checkpoint(end_iters, is_best=False) # todo 待删除
    ...
    def _save_checkpoint(self, cur_iters, is_best=False, is_last=False):
        ...
        for k in list(state_dict.keys()):
            if "Qformer" in k or "llm_proj" in k or "adapter" in k:  # adapter, qformer, 可训练的ln 和 proj以及llm_proj都保存下来
                continue
            elif k in param_grad_dic.keys() and not param_grad_dic[k]:
